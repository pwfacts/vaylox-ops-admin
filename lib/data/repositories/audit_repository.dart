    ...
    for (var row in (salaryRes as List)) {
      logs.add(
        AuditLogEntry(
          ...
          details:
              'Override for ${row['guards']['full_name']}: ${row['manual_override_note'] ?? 'No reason provided'}',
          type: 'SALARY',
        ),
      );
    }
    ...
    for (var row in (attendanceRes as List)) {
      logs.add(
        AuditLogEntry(
          ...
          action: row['approval_status'] == 'APPROVED'
              ? 'Attendance Approved'
              : 'Attendance Rejected',
          details:
              '${row['approval_status']} for ${row['guards']['full_name']} on ${row['attendance_date']}',
          type: 'ATTENDANCE',
        ),
      );
    }
