...
import '../screens/supervisor_bulk_screen.dart'; 
...
class PayrollWizardScreen extends ConsumerStatefulWidget {
  ...
  const PayrollWizardScreen({super.key, required this.unitId, required this.unitName});

  @override
  ConsumerState<PayrollWizardScreen> createState() => _PayrollWizardScreenState();
}

class _PayrollWizardScreenState extends ConsumerState<PayrollWizardScreen> {
  DateTime _selectedMonth = DateTime.now();
  final TextEditingController _unitCalculationDaysController = TextEditingController(text: '30');
  ...
  void dispose() {
    ...
    for (var c in _otControllers.values) {
      c.dispose();
    }
  ...
  TextEditingController _getCalcDaysController(String guardId) {
    return _calcDaysControllers.putIfAbsent(guardId, () => TextEditingController(text: _unitCalculationDaysController.text));
  }

  TextEditingController _getBasicController(String guardId, double initialValue) {
    return _basicControllers.putIfAbsent(guardId, () => TextEditingController(text: initialValue.toStringAsFixed(0)));
  }

  TextEditingController _getPresentDaysController(String guardId, int initialValue) {
    return _presentDaysControllers.putIfAbsent(guardId, () => TextEditingController(text: initialValue.toString()));
  }

  TextEditingController _getOtDaysController(String guardId, int initialValue) {
    return _otDaysControllers.putIfAbsent(guardId, () => TextEditingController(text: initialValue.toString()));
  }

  TextEditingController _getOtBasicController(String guardId, double initialValue) {
    return _otBasicControllers.putIfAbsent(guardId, () => TextEditingController(text: initialValue.toStringAsFixed(0)));
  }

  TextEditingController _getOtAmountController(String guardId) {
    return _otAmountControllers.putIfAbsent(guardId, () => TextEditingController(text: ''));
  }

  TextEditingController _getAdvanceController(String guardId) {
    return _advanceControllers.putIfAbsent(guardId, () => TextEditingController(text: '0'));
  }

  TextEditingController _getUniformController(String guardId) {
    return _uniformControllers.putIfAbsent(guardId, () => TextEditingController(text: '0'));
  }

  TextEditingController _getOtherDed1Controller(String guardId) {
    return _otherDed1Controllers.putIfAbsent(guardId, () => TextEditingController(text: '0'));
  }

  TextEditingController _getOtherDed2Controller(String guardId) {
    return _otherDed2Controllers.putIfAbsent(guardId, () => TextEditingController(text: '0'));
  }

  TextEditingController _getOverrideNoteController(String guardId) {
    return _overrideNoteControllers.putIfAbsent(guardId, () => TextEditingController(text: 'Manual adjustment confirmed'));
  }
        ...
        actions: [
          if (payrollState.activeMonthSlips.isNotEmpty)
            TextButton.icon(
              ...
              label: const Text('Export Excel', style: TextStyle(color: Colors.greenAccent)),
            ),
        ],
      ),
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            Text('Processing for: ${widget.unitName}', style: AppTextStyles.heading2),
            const SizedBox(height: 8),
            Text('Generate salary slips for approved attendance.', style: TextStyle(color: Colors.grey[400])),
            ...
            if (payrollState.error != null)
              Padding(
                ...
                child: Text('Error: ${payrollState.error}', style: const TextStyle(color: AppColors.error)),
              ),
            ...
            if (payrollState.activeMonthSlips.isNotEmpty)
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Generated Slips: ${payrollState.activeMonthSlips.length}', 
        ...
        child: Row(
          ...
          children: [
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Selected Month', style: TextStyle(fontSize: 14, color: Colors.grey)),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const Expanded(
            child: Text('Base Salary Days (Unit Wide):', style: TextStyle(fontWeight: FontWeight.bold)),
          ),
          SizedBox(
            width: 80,
            child: TextFormField(
  ...
  Widget _buildGuardAdjustmentList() {
    ...
    final payrollState = ref.watch(payrollProvider);
    final summaryAsync = ref.watch(unitAttendanceSummaryProvider({
      ...
      data: (guards) => summaryAsync.when(
        data: (summary) => ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final existingSlip = payrollState.activeMonthSlips.firstWhere(
                ...
                children: [
                    ...
                    children: [
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(guard.fullName, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                            Text(guard.designation.replaceAll('_', ' ').toUpperCase(), 
                      ...
                      if (existingSlip != null)
                        IconButton(
                          ...
                          onPressed: () => ref.read(payrollProvider.notifier).printSlip(existingSlip, guard),
                          tooltip: 'Print Salary Slip',
                        ),
                    ],
                  ),
                  const Divider(),
                  if (guardSummary != null)
                    Padding(
                      padding: const EdgeInsets.only(bottom: 12),
                      child: Row(
                        children: [
                          _suggestionChip('Suggest Normal: ${guardSummary['normal']}'),
                          ...
                          if (existingSlip != null)
                            Row(
                              children: [
                                Text('Net: ₹${existingSlip.netPay.toStringAsFixed(0)}', 
                                     style: const TextStyle(color: AppColors.success, fontWeight: FontWeight.bold)),
                                const SizedBox(width: 8),
                                Container(
                                  padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                                  decoration: BoxDecoration(
                                    color: existingSlip.status == 'paid' ? Colors.green.withOpacity(0.2) : Colors.red.withOpacity(0.2),
                                  ...
                                  child: Text(
                                    ...
                                    style: TextStyle(
                                      fontSize: 10, 
                                      color: existingSlip.status == 'paid' ? Colors.green : Colors.redAccent,
                                ...
                                if (existingSlip.status != 'paid')
                                  IconButton(
                                    ...
                                    onPressed: () => ref.read(payrollProvider.notifier).markAsPaid(existingSlip),
                    ...
                    children: [
                      Expanded(
                        child: TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'Calculation Days', hintText: '30/26/etc'),
                          keyboardType: TextInputType.number,
                        ),
                      ),
                      ...
                      Expanded(
                        child: TextFormField(
                          controller: _getBasicController(guard.id, initialBasic),
                          decoration: const InputDecoration(labelText: 'Basic Salary (Manual)', prefixText: '₹'),
                          keyboardType: TextInputType.number,
                        ),
                      ),
                    ],
                  ...
                  const SizedBox(height: 12),
                   Row(
                    children: [
                      Expanded(
                        child: TextFormField(
                          controller: _getPresentDaysController(guard.id, guardSummary['normal'] ?? 0),
                          decoration: const InputDecoration(labelText: 'Present Days (Manual)'),
                          keyboardType: TextInputType.number,
                        ),
                      ),
                      const SizedBox(width: 16),
                      Expanded(
                        child: TextFormField(
                          controller: _getOtDaysController(guard.id, guardSummary['ot'] ?? 0),
                          decoration: const InputDecoration(labelText: 'OT Days (Manual)'),
                          keyboardType: TextInputType.number,
                        ),
                      ),
                    ],
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: TextFormField(
                          controller: _getOtBasicController(guard.id, initialBasic),
                          decoration: const InputDecoration(labelText: 'OT Basic (Manual)', prefixText: '₹'),
                          keyboardType: TextInputType.number,
                        ),
                      ),
                      const SizedBox(width: 16),
                      Expanded(
                        child: TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'Fixed OT Amount', hintText: 'Override calc'),
                          keyboardType: TextInputType.number,
                        ),
                      ),
                    ],
                    ...
                    children: [
                      Expanded(
                        child: TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'Canteen', prefixText: '₹'),
                          keyboardType: TextInputType.number,
                        ),
                      ),
                      const SizedBox(width: 16),
                      Expanded(
                        child: TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'Penalty', prefixText: '₹'),
                          keyboardType: TextInputType.number,
                        ),
                      ),
                    ],
                    ...
                    children: [
                      Expanded(
                        child: TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'Advance', prefixText: '₹'),
                          keyboardType: TextInputType.number,
                        ),
                      ),
                      const SizedBox(width: 16),
                      Expanded(
                        child: TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'Uniform', prefixText: '₹'),
                          keyboardType: TextInputType.number,
                        ),
                      ),
                    ],
                    ...
                    children: [
                      Expanded(
                        child: TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'Other Ded 1', prefixText: '₹'),
                          keyboardType: TextInputType.number,
                        ),
                      ),
                      const SizedBox(width: 16),
                      Expanded(
                        child: TextFormField(
                          ...
                          decoration: const InputDecoration(labelText: 'Other Ded 2', prefixText: '₹'),
                          keyboardType: TextInputType.number,
                        ),
                      ),
                    ],
                    ...
                    children: [
                      Expanded(
                        child: SwitchListTile(
                          title: const Text('PF (12%)', style: TextStyle(fontSize: 12)),
                          value: _getPfToggle(guard.id, guard.isPfEnabled),
                          onChanged: (v) => setState(() => _pfToggles[guard.id] = v),
                        ),
                      ),
                      Expanded(
                        child: SwitchListTile(
                          title: const Text('PT (₹200)', style: TextStyle(fontSize: 12)),
                          value: _getPtToggle(guard.id, guard.isPtEnabled),
                          onChanged: (v) => setState(() => _ptToggles[guard.id] = v),
                        ),
                      ),
                    ],
                  ),
                  ...
                  TextFormField(
                    ...
                    decoration: const InputDecoration(labelText: 'Audit Note / Reason', hintText: 'Explain why days/amount changed'),
                  ),
                  const SizedBox(height: 16),
                  ElevatedButton(
  ...
  Widget _suggestionChip(String label) {
    return Container(
      ...
      child: Text(label, style: const TextStyle(fontSize: 12, color: Colors.blueAccent)),
    );
  }

  Future<void> _runPayrollForGuard(Guard guard) async {
    final calcDays = int.tryParse(_calcDaysControllers[guard.id]?.text ?? '') ?? 
                    int.tryParse(_unitCalculationDaysController.text) ?? 30;
    
    final presentDays = int.tryParse(_presentDaysControllers[guard.id]?.text ?? '');
    final otDays = int.tryParse(_otDaysControllers[guard.id]?.text ?? '');
    
    final basic = double.tryParse(_basicControllers[guard.id]?.text ?? '0') ?? 0;
    final otBasic = double.tryParse(_otBasicControllers[guard.id]?.text ?? '0') ?? 0;
    final otAmount = double.tryParse(_otAmountControllers[guard.id]?.text ?? '');
    
    final canteen = double.tryParse(_canteenControllers[guard.id]?.text ?? '0') ?? 0;
    final penalty = double.tryParse(_penaltyControllers[guard.id]?.text ?? '0') ?? 0;
    final advance = double.tryParse(_advanceControllers[guard.id]?.text ?? '0') ?? 0;
    final uniform = double.tryParse(_uniformControllers[guard.id]?.text ?? '0') ?? 0;
    final other1 = double.tryParse(_otherDed1Controllers[guard.id]?.text ?? '0') ?? 0;
    final other2 = double.tryParse(_otherDed2Controllers[guard.id]?.text ?? '0') ?? 0;
    final note = _overrideNoteControllers[guard.id]?.text;

    if (basic <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please enter a valid Basic Salary')));
      return;
    }
    ...
    await ref.read(payrollProvider.notifier).runPayrollForGuard(
      ...
      overrideBy: SupabaseService().client.auth.currentUser?.email ?? 'Supervisor',
      overrideNote: note,
  ...
  Future<void> _runPayroll() async {
    // This is now handled per guard, or we could add a "Process All" with current values
  }
}
