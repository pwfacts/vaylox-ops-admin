...
class _EnrollmentScreenState extends ConsumerState<EnrollmentScreen> {
  ...
  void _executeEnrollment() {
    ...
    ref.read(guardEnrollmentProvider.notifier).enroll(
      guard: guard,
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(guardEnrollmentProvider, (previous, next) {
      if (next.isSuccess) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Guard enrolled successfully!')));
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ListTile(
                      title: Text(_dob == null ? 'Date of Birth *' : 'DOB: ${DateFormat('yyyy-MM-dd').format(_dob!)}'),
                    ...
                    TextFormField(
                      ...
                      decoration: const InputDecoration(labelText: 'Guard Code (e.g., BH001) *'),
                      validator: (v) => v!.isEmpty ? 'Required' : null,
                    ),
                    _buildUnitSelector(),
                    ...
                    DropdownButtonFormField<DutyShift>(
                      value: _dutyShift,
                      items: DutyShift.values.map((s) => DropdownMenuItem(value: s, child: Text(s.name.toUpperCase()))).toList(),
                    ...
                    SwitchListTile(
                      ...
                      subtitle: const Text('Auto-deduct ₹200 if Gross >= ₹12,000'),
                      value: _isPtEnabled,
  ...
  Widget _buildDocPicker(String label, String key) {
    return ListTile(
      title: Text(label),
      leading: _documents.containsKey(key) ? const Icon(Icons.check_circle, color: Colors.green) : const Icon(Icons.camera_alt),
